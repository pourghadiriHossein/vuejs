...
export class User {
  ...
  static async updateProfile(id: number,username: string, email: string, password:string, avatar: File) {
    ...
    data.append('password', password);
    data.append('avatar', avatar);
    const response = await api.post<AxiosResponse>(`api/update-my-profile/${id}`,data);
    if(response.status == 200) {
      return response;
    }
  ...
  static async allUserForAdmin(page: number) {
    const response = await api.get<FetchResponse<User>>(`api/all-users?page=${page}`);
    if (response.status == 200){
      return response;
    }
  ...
  static async createUserByAdmin(username: string, email: string, password:string, avatar: File, role: string) {
    const data = new FormData();
    data.append('name', username);
    data.append('email', email);
    data.append('password', password);
    ...
    const response = await api.post<AxiosResponse>('api/create-user-by-admin',data);
    if(response.status == 200) {
      return response;
    }
  ...
  static async updateUserByAdmin(id: number, username: string, email: string, password:string, avatar: File, role: string) {
    const data = new FormData();
    data.append('name', username);
    data.append('email', email);
    data.append('password', password);
    data.append('avatar', avatar);
    data.append('role', role);
    const response = await api.post<AxiosResponse>(`api/update-user-by-admin/${id}`,data);
    if(response.status == 200) {
      return response;
    }
    throw Error('Created Failed');
